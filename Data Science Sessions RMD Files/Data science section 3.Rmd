...
A function is a mathematical relationship between a set of inputs and outputs. Functions are subject to constraints such as a range or set of possible inputs in addition to a range of possible outputs. Functions are universally denoted as f(x). In some instances, f(x)= is interchangable with y=, it really just depends on the operations at hand. 

In our previous lessons, we examined mechanics but going forward, mecahnics will be a tool to work through more concrete concepts and their applications. 
...
We split up our ordered pairs into their x components and y components. Notice in the y components, we omitt the additional -3. We have two distinct x values (0 and 2) that yield the y value of -3, however, we would no longer have a relation if the same x value yields two different y values such as the example below: 

$$
(6,10),(-7,3),(0,4),(6, -4)
$$

Without looking at the x and y components seperate, we can see that the x value of 6 yields a y value of 10 and -4. This contradicts the definition of a relation. 
...
Finding the domain and range of a function requires some knowledge of elementary rules. Some of the most popular rules for functions are:
